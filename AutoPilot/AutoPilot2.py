...
parser.add_argument('-nruns', '--maxIterations', type=int,
                    default=999999, help='Number of runs to take', required=False)

parser.add_argument('-run','--runNumber',type=int,
                    default=0, help='Run number', required=False)
...
while (AutoPilotStatus == 1 and iteration < maxRuns):
    ...
    if iteration % 5 == 0:
        if IsTelescope:
            StartSeconds, StopSeconds = GetStartAndStopSeconds(30, 10)  # 33,22
        else:
            StartSeconds, StopSeconds = GetStartAndStopSeconds(50, 22)
    ...
    if not Debug and IsTelescope:
        tp.start_ots(RunNumber, False)
    ...
    if pf.QueryGreenSignal(True):
        ...
        ETLTimestamp = (datetime.now() - datetime.strptime("2000-01-01 00:00:00",
                        "%Y-%m-%d %H:%M:%S")).total_seconds()  # - 3600 ### For daylight saving time
